...
# Add this new class to your existing batched_sampler.py file

class SingleProcessDynamicSampler(Sampler):
    """
    A non-distributed "twin" of DynamicDistributedSampler.
    It mimics the behavior required by DynamicBatchSampler for single-process execution.
    It does NOT inherit from DistributedSampler and makes no calls to torch.distributed,
    thus avoiding the initialization error.
    """
    def __init__(self, dataset, shuffle: bool = False, seed: int = 0):
        ...
        # These will be set by DynamicBatchSampler via update_parameters
        self.resolution_idx = None
        self.image_num = None
    ...
    def __iter__(self):
        """
        Yields a sequence of (index, resolution_idx, image_num).
        """
        if self.shuffle:
            g = torch.Generator()
        ...
        for idx in indices:
            # Yield the same tuple format as DynamicDistributedSampler
            yield (idx, self.resolution_idx, self.image_num)

    def update_parameters(self, resolution_idx, image_num):
        """
        Updates dynamic parameters, called by DynamicBatchSampler.
        """
        self.resolution_idx = resolution_idx
        self.image_num = image_num
