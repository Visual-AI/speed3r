...
class PointLoss(nn.Module):
    ...
    def prepare_segformer(self):
        ...
        self.segformer.load_state_dict(torch.load('/path_to/segformer.b0.512x512.ade.160k.pth', map_location=torch.device('cpu'), weights_only=False)['state_dict'])
        self.segformer = self.segformer.to('cuda')
...
class Pi3LossDistill(nn.Module):
    def __init__(
        self,
        train_conf=False,
    ):
    ...
    def normalize_gt(self, gt):
        local_points = torch.stack([view['local_points'] for view in gt], dim=1)
        masks = torch.stack([view['valid_mask'] for view in gt], dim=1)
        poses = torch.stack([view['camera_pose'] for view in gt], dim=1)
        B, N, H, W, _ = local_points.shape

        # global_points = torch.einsum('bnij, bnhwj -> bnhwi', poses, homogenize_points(local_points))[..., :3]
        ...
        camera_poses_normalized = poses.clone()
        camera_poses_normalized[..., :3, 3] /= norm_factor.view(B, 1, 1)
    ...
    def normalize_pred(self, pred, gt):
        ...
        # global_points = torch.einsum('bnij, bnhwj -> bnhwi', camera_poses, homogenize_points(local_points))[..., :3]
    ...
    def forward(self, pred, gt_raw):
        ...
        # Feat Loss
        # feat_loss = self.feat_loss(pred['mid_hidden'], gt['mid_hidden'][:,:,5:,:])
        # final_loss += feat_loss
        # details.update({'feat_loss': feat_loss})
